...
var sd=new Date(update_options.startDate);var nextDay=Date(sd.getFullYear(),sd.getMonth(),sd.getDate()+1);if(pr_el==='setStartDate'&&typeof(pr_el)==='string'){update_options.startDate=convertDateUTC(convertFormatDf(options))}
...
function convertDateUTC(pr_date_utc){var date=new Date(pr_date_utc);var date_utc=Date(date.getFullYear(),date.getMonth(),date.getDate());if(date_utc===0){date_utc=null};return date_utc;}
...
if(pr_in===null&&pr_out===null){pr_in=settings.startDate;var date=new Date(pr_in);pr_out=Date(date.getFullYear(),date.getMonth(),date.getDate()+1);}
...
var newDate=new Date(Date(date.getFullYear(),(date.getMonth()+i_num)));tswitch[i_num].innerHTML=settings.titleMonths[newDate.getMonth()]+' '+newDate.getFullYear();var dataDays=[];var dataUTCDate=[];var days=[];var nextDate=Date(date.getFullYear(),(date.getMonth()+i_num));var date=new Date(nextDate)
while(Date(date.getFullYear(),(date.getMonth()))===nextDate){days.push(date.getDay());dataDays.push(date.getDate());dataUTCDate.push(Date(date.getFullYear(),date.getMonth(),date.getDate()))
...
var CI_Arrow=settings.dateCheckIn||toDay;var ci_d=new Date(CI_Arrow);var co_n=Date(ci_d.getFullYear(),ci_d.getMonth(),ci_d.getDate()+settings.limitDateRanges)
...
var d=new Date(dataUTC[1]);var limitdate=Date(d.getFullYear(),d.getMonth(),d.getDate()-settings.limitDateRanges);data_utc_in=get_utc;data_utc_out=dataUTC[1];if(get_utc===settings.startDate&&$(this).hasClass('t-start')===false&&dataUTC[0]>=settings.startDate){data_utc_out=dataUTC[0];}
...
var nd_1=Date(nd.getFullYear(),nd.getMonth(),nd.getDate());var limitday=Date(nd.getFullYear(),nd.getMonth(),nd.getDate()-settings.limitDateRanges);if(el_hover<=limitday){return;}
while(el_hover<nd_1){nd_1=Date(nd.getFullYear(),nd.getMonth(),nd.getDate()-numDay);var t_this=$(pr_el).parents('.t-check-in').find('[data-t-date="'+nd_1+'"]')[0];if(t_this!=undefined){t_this.className=t_this.className.replace(' t-range-limit','')+' t-range-limit';}
numDay++;if(numDay>1000){return;}}}
if($(pr_el).parents('.t-check-out').hasClass('t-check-out')===true){var nd=new Date(pr_date_utc[0]);var nd_1=Date(nd.getFullYear(),nd.getMonth(),nd.getDate());var limitday=Date(nd.getFullYear(),nd.getMonth(),nd.getDate()+settings.limitDateRanges);if(el_hover>=limitday){return;}
while(el_hover!=nd_1){nd_1=Date(nd.getFullYear(),nd.getMonth(),nd.getDate()+numDay);var t_this=$(pr_el).parents('.t-check-out').find('[data-t-date="'+nd_1+'"]')[0];if(t_this!=undefined){t_this.className=t_this.className.replace(' t-range-limit','')+' t-range-limit';}
...
var limitPrevMonth=Date(df_toDay.getFullYear(),df_toDay.getMonth()-settings.limitPrevMonth);var limitNextMonth=Date(end_Date.getFullYear(),end_Date.getMonth())||Date(df_toDay.getFullYear(),df_toDay.getMonth()+numCalendar+settings.limitNextMonth);var newDate=new Date(pr_data_utc)
...
if(Date(y,m)>limitPrevMonth){m=m-1;setDaysInMonth(pr_el,Date(y,m))}}
tArrow[tArrow.length-1].onclick=function(e){e.stopPropagation()
if(Date(y,m+numCalendar)<limitNextMonth){m=m+1;setDaysInMonth(pr_el,Date(y,m))}}}
var Arrow_2=pr_el.find('.t-arrow');if(Date(y,m+numCalendar)!=limitNextMonth&&Date(y,m)===limitPrevMonth){Arrow_2[Arrow_2.length-1].className='t-arrow t-next'
Arrow_2[0].className='t-arrow t-prev t-disabled'}else{Arrow_2[0].className='t-arrow t-prev'}
if(Date(y,m+numCalendar)===limitNextMonth||new Date(Date(y,m+numCalendar)).getMonth()===new Date(dataUTC[0]).getMonth()+1){Arrow_2[Arrow_2.length-1].className='t-arrow t-next t-disabled'}
if(Date(y,m+numCalendar)<limitNextMonth){Arrow_2[Arrow_2.length-1].className='t-arrow t-next'}
